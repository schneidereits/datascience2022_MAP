...
fc_clean <- fc %>% 
  mutate(soil_type = str_replace(soil_type, "Soil_Type", ""),
         soil_type = if_else(soil_type %in% c(1,4), "Cathedral", soil_type),
         soil_type = if_else(soil_type==3, "Haploborolis", soil_type),
         soil_type = if_else(soil_type %in% c(2,5,6), "Vanet", soil_type),
         soil_type = if_else(soil_type==7, "Gothic", soil_type),
         soil_type = if_else(soil_type==8, "Limber", soil_type),
         soil_type = if_else(soil_type %in% c(10,11,13,18,26,31,32,33), "Catamount", soil_type),
         ...
         soil_type = if_else(soil_type %in% c(16,17,19,20,21,23,35), "Cryaquolis", soil_type),
         soil_type = if_else(soil_type %in% c(9,22,23,24,25,27,28,31,32,33,38), "Leighcan", soil_type),
         soil_type = if_else(soil_type %in% c(34,35,36,37,39,40), "Cryorthents", soil_type),
